...
const register = async(req, res) => {
    ...
    const request = await user.save();            // save user in database


    if (!request) {
        // no data returned, return the error
        return res
            .status(400)
            .json(err);
    } else {
        // otherwise return new user token
...
const login = async(req, res) => {
    ...
    passport.authenticate('local', (err, user, info) => {
        if (err) {
            // error authenticating -> return error
            return res
                .status(404)
                .json(err);
        };

        // otherwise, continue to authorization and generate a token
        if (user) {
